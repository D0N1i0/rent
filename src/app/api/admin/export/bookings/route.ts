...
export async function GET(req: NextRequest) {
  ...
  for (const b of bookings) {
    lines.push(row([
      ...
      b.durationDays,
      b.pricingTier,
      Number(b.basePricePerDay).toFixed(2),
      Number(b.subtotal).toFixed(2),
      Number(b.extrasTotal).toFixed(2),
      Number(b.pickupFee).toFixed(2),
      Number(b.dropoffFee).toFixed(2),
      Number(b.discount).toFixed(2),
      `${Math.round(Number(b.vatRate) * 100)}%`,
