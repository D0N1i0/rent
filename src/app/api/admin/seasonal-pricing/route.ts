// src/app/api/admin/seasonal-pricing/route.ts
...
export async function GET() {
  ...
  const items = await prisma.seasonalPricing.findMany({
...
export async function POST(req: NextRequest) {
  ...
  if (parsed.data.isActive) {
    const overlapMsg = await checkOverlap(parsed.data.carId, start, end);
    if (overlapMsg) return NextResponse.json({ error: overlapMsg }, { status: 409 });
  }

  const item = await prisma.seasonalPricing.create({
    data: {
